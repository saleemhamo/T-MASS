...
def evaluate_model_on_test_data(config, model, tokenizer, test_data, video_features_cache, k=10, limit=None):
    ...
    for i, (_, row) in tqdm(enumerate(test_data.iterrows()), total=total_queries, desc="Evaluating queries"):
        ...
        top_videos = find_top_k_matches(config, query, model, tokenizer, video_features_cache, k)
