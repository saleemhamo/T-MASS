...
class AllConfig(Config):
    ...
    def parse_args(self):
        ...
        parser.add_argument('--attention_temperature', type=float, default=0.01,
                            help='Temperature for softmax (used in attention pooling only)')
        ...
        parser.add_argument("--datetime", type=str, default=None,
                            help='to be specificed for loading pre-trained checkpoint ')
        parser.add_argument('--stochasic_trials', type=int, default=20,
                            help='perform [stochastic trials] to compute the averaged text embedding at validation')
        parser.add_argument('--gpu', type=str, default=None, help="gpu id")
        parser.add_argument('--support_loss_weight', type=float, default=0.0,
                            help='compute the contrastive between pooled-video and support text embedding, default=0.')
        parser.add_argument('--batch_size_split', type=int, default=None,
                            help="split integer for batch-wise bmm, larger to save more memory. Default=None, automatically split into 10-sample batches")
        parser.add_argument('--chunk_size', type=int, default=128,
                            help="split integer for batch-wise torch.norm, larger to save more memory. Default=128")
        parser.add_argument('--noloss_record', action='store_true', default=False,
                            help='if specified, no loss values will be recorded to speed up training')
        parser.add_argument('--save_memory_mode', action='store_true', default=False,
                            help='if specified, will use sim_matrix_inference_stochastic_light_allops() at eval no matter of the dataset')
        parser.add_argument('--raw_video', action='store_true', default=False,
                            help='For Charades dataest. if specified, will load video format of .mp4')
        parser.add_argument('--skip_eval', action='store_true', default=False,
                            help='If specified, will not conduct validation() per epoch but only save ckpts')
        parser.add_argument('--DSL', action='store_true', default=False, help='If specified, will normalize use DSL')
        parser.add_argument('--stochastic_prior', type=str, default='uniform01', choices=['uniform01', 'normal'],
